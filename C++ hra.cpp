...
public:
    ...
    Character(string d, string j, int mbv, int sv, int a, int pr)
        : diagnoza(d), jmeno(j), max_vedomi(mbv), vedomi(mbv), max_vule(sv), vule(sv), agresivita(a), presvedcivost(pr) {}
...
int main(){
    vector<Character> characters {
            Character("\nschizofrenie", "Daniel Paul Schreber", 75, 55, 45, 65),
            Character("\nBipolarni afektivni porucha (BAP)", "Virginia Woolf", 70, 65, 50, 55),
            Character("\nTezka depresivni porucha", "Esther Greenwood", 65, 60, 35, 80),
            Character("\nDisociativni porucha identity (DID)", "Tyler Durden", 50, 60, 60, 70)
    };
